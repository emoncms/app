...
function get_heatpump_stats($feed,$app,$start,$end,$starting_power) {
    ...
    if (!isset($app->config->heatpump_flowT) || $app->config->heatpump_flowT<1) return false;
    if (!isset($app->config->heatpump_returnT) || $app->config->heatpump_returnT<1) return false;
    if (!isset($app->config->heatpump_outsideT) || $app->config->heatpump_outsideT<1) return false;
    if (!isset($app->config->heatpump_elec) || $app->config->heatpump_elec<1) return false;
    if (!isset($app->config->heatpump_heat) || $app->config->heatpump_heat<1) return false;
    ...
    for ($z=0; $z<count($elec_data); $z++) {
        ...
        if ($time<$elec_meta->end_time) {
            ...
            if (!is_null($heat_data[$z])) $heat = $heat_data[$z];        
            if (!is_null($flowT_data[$z])) $flowT = $flowT_data[$z];
            if (!is_null($returnT_data[$z])) $returnT = $returnT_data[$z];
            if (!is_null($outsideT_data[$z])) $ambientT = $outsideT_data[$z];

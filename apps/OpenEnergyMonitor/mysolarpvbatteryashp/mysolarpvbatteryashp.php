...
<section id="app-block" style="display:none" class="block">
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="text-xs-center">
            <h5 class="electric-title mb-0 text-md-larger text-light"><span class="heat_pump_use_title"><?php echo _('HEAT PUMP USE') ?></span></h5>
            <h2 class="power-value display-md-3 display-lg-2 my-0" style="color:#f33;"><span class="heatpumpnow">-</span><span class="power-unit"></h2>
        </div>
    </div>
...
config.app = {
    ...
    // Heat pump feed
    "heat_pump":{"type":"feed", "autoname":"heat_pump", "engine":"5", "description":"Building heat pump consumption in watts"},
    ...
    "heat_pump_kwh":{"optional":true, "type":"feed", "autoname":"heat_pump_kwh", "engine":"5", "description":"Heat pump useage energy in kWh"},
...
function livefn()
{
    ...
    var heat_pump_now = parseInt(feeds[config.app.heat_pump.value].value);
    ...
    if (autoupdate) {
        ...
        timeseries.append("heat_pump",updatetime,heat_pump_now);
        timeseries.trim_start("heat_pump",view.start*0.001);
    ...
    if(powerUnit === 'kW') {
        ...
        heat_pump_now = as_kw(heat_pump_now)
    ...
    $(".usenow").html(use_now);
    $(".heatpumpnow").html(heat_pump_now);
...
function load_powergraph() {
    ...
    if (reload) {
        ...
        timeseries.load("heat_pump",feed.getdata(config.app.heat_pump.value,view.start,view.end,view.interval));
    ...
    for (var z=0; z<timeseries.length("solar"); z++) {
        ...
        if (timeseries.value("heat_pump",z)!=null) {
            heat_pump_now = timeseries.value("heat_pump",z);
            last_use = time;
        }
        ...
        if ((time-last_solar)<timeout && (time-last_use)<timeout && (time-last_charge)<timeout && (time-last_discharge)<timeout) {
            ...
            total_heat_pump_kwh += (heat_pump_now*interval)/(1000*3600);
            ...
            use_data.push([time,use_now-heat_pump_now]);
            heat_pump_data.push([time,heat_pump_now]);
    ...
    if(config.app.is_dc_battery.value)
    {
        ...
        if(config.app.heat_pump.value)
            powerseries.push({data:heat_pump_data, label: "Heat Pump", color: "#f33", stack:3, lines:{lineWidth:0, fill:0.8}});
    }
    else
    {
        ...
        if(config.app.heat_pump.value)
            powerseries.push({data:heat_pump_data, label: "Heat Pump", color: "#f33", stack:2, lines:{lineWidth:0, fill:0.8}});
    }

    ...
    <script>
        ...
        config.app = {
            ...
            "tariff": {
                ...
                "options": ["AGILE-18-02-21", "AGILE-22-07-22", "AGILE-22-08-31", "AGILE-23-12-06", "AGILE-VAR-22-10-19", "AGILE-FLEX-22-11-25", "AGILE-24-10-01"]
            },
        ...
        function graph_load() {
            ...
            if (import_kwh.length > 1) {
                for (var z = 1; z < import_kwh.length; z++) {
                    ...
                    if (solarpv_mode) {
                        ...
                        monthly_data[startOfMonth]["import"] += kwh_import
                        monthly_data[startOfMonth]["cost_import"] += kwh_import * cost_import
                        monthly_data[startOfMonth]["cost_import_go"] += kwh_import * cost_import_go
                    }
                }
            }
            ...
            if (Object.keys(monthly_data).length > 1 && !solarpv_mode) {
                ...
                var monthly_sum_cost_import = 0;
                var monthly_sum_cost_import_go = 0;
                ...
                for (var month in monthly_data) {
                    ...
                    let agile_cost = monthly_data[month]["cost_import"]*vat;
                    let go_cost = monthly_data[month]["cost_import_go"]*vat;
                    let agile_unit_cost = 100*(agile_cost / kwh);
                    let go_unit_cost = 100*(go_cost / kwh);

                    monthly_out += "<tr>";
                    monthly_out += "<td>" + d.getFullYear() + " " + months[d.getMonth()] + "</td>";
                    monthly_out += "<td>" + monthly_data[month]["import"].toFixed(1) + " kWh</td>";
                    monthly_out += "<td>£" + agile_cost.toFixed(2) + "</td>";
                    monthly_out += "<td>" + agile_unit_cost.toFixed(1) + " <span style='font-size:12px'>p/kWh</span></td>";
                    monthly_out += "<td>£" + go_cost.toFixed(2) + "</td>";
                    monthly_out += "<td>" + go_unit_cost.toFixed(1) + " <span style='font-size:12px'>p/kWh</span></td>";
                    ...
                    monthly_sum_cost_import += agile_cost;
                    monthly_sum_cost_import_go += go_cost;
                }
                ...
                monthly_out += "<td>£" + monthly_sum_cost_import.toFixed(2) + "</td>";
                monthly_out += "<td>" + ((monthly_sum_cost_import / monthly_sum_kwh) * 100).toFixed(1) + " <span style='font-size:12px'>p/kWh</span></td>";
                monthly_out += "<td>£" + monthly_sum_cost_import_go.toFixed(2) + "</td>";
                monthly_out += "<td>" + ((monthly_sum_cost_import_go / monthly_sum_kwh) * 100).toFixed(1) + " <span style='font-size:12px'>p/kWh</span></td>";
                monthly_out += "</tr>";
